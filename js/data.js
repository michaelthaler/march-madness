...
var DEFAULT_RESULTS = [
  ...
  // East Region
  [3, 'East', 1, 5, 1],    // Duke def. St. John's
  [3, 'East', 2, 3, 2],    // UConn def. Michigan St.
  // Midwest Region
  [3, 'Midwest', 1, 4, 1], // Michigan def. Alabama
  [3, 'Midwest', 2, 6, 6], // Tennessee def. Iowa St. (upset)
...
function getFFGames(regions, results) {
  ...
  for (var s = 0; s < 2; s++) {
    var semi = games.semis[s];
    if (semi.winner && semi._region1 && semi._region2) {
      var winRegion = (semi.winnerSide === 2) ? semi._region2 : semi._region1;
      if (s === 0) games.championship._region1 = winRegion;
      else games.championship._region2 = winRegion;
    }
  }
